 ...
 * ***************************************************
 * HW2_prog2.cpp
 * Creates N buyer threads and 4 provider threads, where
 * buyer threads 'take' the number out of a queue that
 * is filled by provider threads
 * 
 * Code from the examples given in class was used as a 
 * template for this assignmen
 * ***************************************************/
...
int main(int argc, char **argv)
{
  //for(int m = 0; m < argc; m++)
    ...
    BUYER_NUM = strtol(argv[1], NULL, 10) + 2;
    const int BUY_NUM = BUYER_NUM;
    pthread_t pID[PROVIDER_NUM];
    pthread_t bID[BUY_NUM];
    void *thread_result;
    int state1, state2;
    ...
    for(long int j = 0; j < PROVIDER_NUM; j++)
    {    
      pthread_create(&pID[j], NULL, thread_Insert, (void*)j);
